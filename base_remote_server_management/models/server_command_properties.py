    ...
    def _compute_property_value(self):
        for prop in self:
            if prop.command:
                ...
                key = paramiko.RSAKey.from_private_key_file(prop.base_remote_server_id.private_key_path)
                ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                ssh.connect(prop.base_remote_server_id.server_ip, port=prop.base_remote_server_id.ssh_port, username=prop.base_remote_server_id.ssh_user, pkey=key)
